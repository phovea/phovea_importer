    ...
    static async importTable(editors, $root, header, data, name) {
      ...
      `);
        const configPromises = header.map((name, i) => {
            return ValueTypeUtils.guessValueType(editors, name, i, data, (row) => row[i]);
        });
        ...
        const config = await Promise.all(header.map(async (name, i) => {
            ...
            const markup = await ValueTypeUtils.createTypeEditor(editors, guessedEditors[i], value);
            return {
                column: i,
                name,
                color: '#DDDDDD',
        ...
        $rowsEnter.select('select').on('change', ValueTypeUtils.updateType(editors));
