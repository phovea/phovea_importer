...
export class ValueTypeUtils {
    ...
    static string_() {
        return {
            isType: () => 1,
            parse: ValueTypeUtils.parseString,
            guessOptions: ValueTypeUtils.guessString,
            edit: ValueTypeUtils.editString,
    ...
    static categorical() {
        return {
            isType: ValueTypeUtils.isCategorical,
    ...
    static guessNumerical(def, data, accessor) {
        ...
        data.forEach((row) => {
            ...
            if (ValueTypeUtils.isMissingNumber(raw)) {
                return; //skip
            }
    ...
    static isNumerical(name, index, data, accessor, sampleSize) {
        ...
        for (let i = 0; i < testSize; ++i) {
            const v = accessor(data[i]);
            if (ValueTypeUtils.isMissingNumber(v)) {
                continue; //skip empty samples
            }
            validSize++;
    ...
    static numerical() {
        return {
            isType: ValueTypeUtils.isNumerical,
            parse: ValueTypeUtils.parseNumerical,
            guessOptions: ValueTypeUtils.guessNumerical,
            edit: ValueTypeUtils.editNumerical,
            getOptionsMarkup: ValueTypeUtils.singleOption
        };
    }
    /**
